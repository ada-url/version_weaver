...
std::string incrementVersion(const std::string& version) {
  ...
  if (std::regex_match(version, match, version_regex)) {
      ...
      if (!preRelease.empty()) {
          return match[1].str() + "." + match[2].str() + "." + match[3].str() + "-" + preRelease + ".0";
      }
  ...
  if (std::regex_match(version, match, version_regex)) {
      ...
      if (!preRelease.empty()) {
          if (preRelease.find("beta") != std::string::npos) {
              return match[1].str() + "." + match[2].str() + "." + match[3].str() + "-alpha.0";
...
bool satisfiesConstraint(const std::string &candidate, const std::string &op, const std::string &ver) {
...
std::string computeTildeUpperBound(const std::string &version) {
  ...
  std::string upper = std::to_string(major) + "." + std::to_string(minor + 1) + ".0";
  return upper;
}
...
std::optional<std::string> minimum(const std::string &range) {
  ...
  // If the entire expression is just "*" (possibly with surrounding spaces), return "0.0.0" directly.
  std::regex star_regex(R"(^\s*\*\s*$)");
  if (std::regex_match(range, star_regex))
      return "0.0.0";

  // Support for the dash operator ("A - B" form)
  std::regex dash_regex(R"(^\s*([\d]+(?:\.[\d]+){0,2})\s*-\s*([\d]+(?:\.[\d]+){0,2})\s*$)");
  ...
  for (; subIt != subEnd; ++subIt) {
      ...
      if (start != std::string::npos)
          subRange = subRange.substr(start);
      auto endpos = subRange.find_last_not_of(" \t\n\r");
      if (endpos != std::string::npos)
          subRange = subRange.substr(0, endpos + 1);
      ...
      for (; it != itEnd; ++it) {
          ...
          if (op == "^") {
              // For caret, add a lower constraint ">= ver" and an upper constraint based on caret rules.
              lowerConstraints.push_back({">=", ver});
              std::string upperBound = computeCaretUpperBound(ver);
              if (!upperBound.empty())
                  upperConstraints.push_back({"<", upperBound});
          } else if (op == "~") {
              // For tilde, add a lower constraint ">= ver" and an upper constraint based on tilde rules.
              lowerConstraints.push_back({">=", ver});
      ...
      // If there are no constraints in the sub-range (e.g., "1.0.x", "1.x", "=1.0.0", etc.),
      // then the candidate is the normalized form of the sub-range.
      if (!std::regex_search(subRange, anyConstraint)) {
          ...
          for (auto &lc : lowerConstraints) {
              // For ">" operator, use incrementVersion; for ">=" simply use the version.
              std::string cur = (lc.first == ">") ? incrementVersion(lc.second) : lc.second;
              if (candidate.empty() || compareSemVer(candidate, cur))
                  candidate = cur;
          }
      } else {
          candidate = "0.0.0";
      ...
      for (auto &uc : upperConstraints) {
          ...
          // Special case: if the constraint is "<0.0.0-beta" and the candidate is "0.0.0",
          // change the candidate to "0.0.0-0".
  ...
  if (validCandidates.empty())
      return std::nullopt;
  return *std::min_element(validCandidates.begin(), validCandidates.end(), compareSemVer);
}
